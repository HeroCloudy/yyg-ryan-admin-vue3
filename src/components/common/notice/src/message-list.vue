<!--
 @name: message-list.vue
 @description: 消息列表组件
 @author: 程序员优雅哥 yygcoder
 @time: 2023/12/20 10:45
-->
<template>
      ...
      <ul v-if="tab.contents && tab.contents.length">
        ...
        >
          <el-row justify="center" align="middle">
            <el-col :span="4" @click="() => handleClickAvatar(item.avatar)" v-if="item.avatar">
              <el-avatar v-bind="Object.assign({ size: 'small' }, item.avatar)" />
            </el-col>
            <el-col :span="20" class="px-3" @click="() => handleClickItem(item)">
              <!-- 消息 -->
              <el-row align="middle" class="mb-2 flex-nowrap!">
                <!-- 消息标题 -->
                <div class="line-clamp-1 text-base">
                  {{ item.title }}
                </div>
                <!-- 标签 -->
  ...
  <div class="w-full flex justify-around border-t">
    <div
      class="flex flex-1 cursor-pointer items-center justify-center border-r py-3 text-gray-500 hover:bg-sky-100"
      v-for="(action, index) in actions"
      @click="action.click"
      :key="index"
    >
      <yyg-icon v-if="action.icon" :icon="action.icon" :color="action.color"></yyg-icon>
